...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should update user balance and empty the cart on success", async () => {
      ...
      const walletMoney = userOneFinal.walletMoney;
      await cartService.checkout(userOneFinal);
      let totalCost = 0;
      cartWithProductsUserOne.cartItems.forEach((item) => { totalCost += item.product.cost * item.quantity });
      ...
      expect(userOneFinal.walletMoney).toEqual(walletMoney - totalCost)
    });
  });
});
